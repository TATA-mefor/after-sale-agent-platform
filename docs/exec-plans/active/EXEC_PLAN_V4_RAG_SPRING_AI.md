...
opt-in integration docs are completed. V4.4.1 Policy Ingestion domain / status / repository foundation is
completed. Policy Ingestion runtime and Hybrid RAG runtime remain planned.
...
Status: active. V4.4.1 Policy document / chunk domain and ingestion run model is completed. Chunking, checksum
deduplication, embedding pipeline, vector repository writes, ingestion API, and RAG / HYBRID retrieval remain planned.
